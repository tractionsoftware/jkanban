    ...
    this.addBoards = function(boards, isInit) {
      ...
      for (var boardkey in boards) {
        ...
        boardNode.dataset.order = self.container.childNodes.length + 1;
          ...
          // set eth class (class name for the entry header tags) [by Takashi]
          //if (board.ehtClassName !== "" && board.ehtClassName !== undefined) {
          //  boardNode.dataset.eht = board.ehtClassName + " eht";
          //}
          // set data-displayname="displayName of the board" [by Takashi]
          boardNode.dataset.displayname = board.displayname;
          // set data-includeLabelname="todo" (for example) [by Takashi]
          if (board.includelabelname !== "" && board.includelabelname !== undefined) {
            boardNode.dataset.includelabelname = board.includelabelname;
          }
          // set data-excludeLabelname="todo" (for example) [by Takashi]
          if (board.excludelabelname !== "" && board.excludelabelname !== undefined) {
            boardNode.dataset.excludelabelname = board.excludelabelname;
          }


// Diabled board widths [by Takashi]
// Let board widths be set by CSS generated by JavaScript
//          if (self.options.responsivePercentage) {
...
//          boardNode.style.marginRight = self.options.gutter;
        ...
        // header board class names (replaced "class" with "headerClass") [by Takashi]
        ...
        // board priority [by Takashi]
        if (board.bodyPriority !== "" && board.bodyPriority !== undefined)
          contentBoard.dataset.priority = board.bodyPriority;
        else contentBoard.dataset.priority = "";
        ...
        for (var itemkey in board.item) {
          ...
          // Added the right-click event listner [by Takashi]
          // to display a mini menu. See JPBO25710.
    ...
    function __onclickHandler(nodeItem, clickfn) {
      nodeItem.addEventListener("click", function(e) {
        // Prevent the click event for kanban items to allow
        // clicking on title and archive links on kanban items.
        // See JPBO25709. [by Takashi]
        //e.preventDefault();
